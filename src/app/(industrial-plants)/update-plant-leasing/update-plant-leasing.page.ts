...
@Component({
  selector: 'app-update-plant-leasing',
  templateUrl: './update-plant-leasing.page.html',
  styleUrls: ['./update-plant-leasing.page.scss'],
  standalone:false,
})
export class UpdatePlantLeasingPage implements OnInit {
    ...
    this.plantservice.getleasingData().subscribe({
      next: (data) => {
       ...
       // Store fetched data in carData property
        this.filteredCarSaleData = this.carSaleData.filter((item: {plant_leasing_ad_sale_id: string; }) => item.plant_leasing_ad_sale_id === this.adsId);
...
async saveUser(): Promise<void> {
  ...
  if (this.userForm.valid) {
    ...
    // Now you can use fileList for your API call
    this.plantservice.updateLeasingBusiness(formData).subscribe(
      (response) => {
