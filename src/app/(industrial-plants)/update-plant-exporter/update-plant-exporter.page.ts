...
@Component({
  selector: 'app-update-plant-exporter',
  templateUrl: './update-plant-exporter.page.html',
  styleUrls: ['./update-plant-exporter.page.scss'],
  standalone:false,
})
export class UpdatePlantExporterPage implements OnInit {
    ...
    fetchCarSale() {
      ...
      this.plantservice.getexporterData().subscribe({
        next: (data) => {
          ...
          this.filteredCarSaleData = this.carSaleData.filter((item: {plant_exporter_ad_sale_id: string; }) => item.plant_exporter_ad_sale_id === this.adsId);
...
updateSelectOptions() {
  ...
  async saveUser(): Promise<void> {
    ...
    if (this.userForm.valid) {
      ...
      this.plantservice.updateExporterBusiness(formData).subscribe(
        (response) => {
