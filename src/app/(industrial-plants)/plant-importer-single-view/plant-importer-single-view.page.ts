...
@Component({
  selector: 'app-plant-importer-single-view',
  templateUrl: './plant-importer-single-view.page.html',
  styleUrls: ['./plant-importer-single-view.page.scss'],
  standalone:false,
})
export class PlantImporterSingleViewPage implements OnInit {
  ...
  filterCarData() {
    ...
    this.filterData = this.importerData.filter(car =>
      (importerId.length === 0 || importerId === car.plant_importer_ad_sale_id) // Exact match check
    );
