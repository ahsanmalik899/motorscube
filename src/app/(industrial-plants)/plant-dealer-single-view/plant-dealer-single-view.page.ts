...
@Component({
  selector: 'app-plant-dealer-single-view',
  templateUrl: './plant-dealer-single-view.page.html',
  styleUrls: ['./plant-dealer-single-view.page.scss'],
  standalone:false,
})
export class PlantDealerSingleViewPage implements OnInit {
    ...
    fetchdealershipData() {
      ...
      this.PlantsService.getdealershipData().subscribe({
        next: (data) => {
    ...
    filterCarData() {
      ...
      this.filterData = this.dealershipData.filter(car => {
        ...
        return (dealerId.length === 0 || car.plant_dealership_ad_sale_id === dealerId);
      });
