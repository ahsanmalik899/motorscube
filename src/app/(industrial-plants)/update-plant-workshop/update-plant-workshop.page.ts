...
@Component({
  selector: 'app-update-plant-workshop',
  templateUrl: './update-plant-workshop.page.html',
  styleUrls: ['./update-plant-workshop.page.scss'],
  standalone:false,
})
export class UpdatePlantWorkshopPage implements OnInit {
    ...
    fetchCarSale() {
      ...
      this.plantservice.getworkshopData().subscribe({
        next: (data) => {
          ...
          this.filteredCarSaleData = this.carSaleData.filter((item: {plant_workshop_ad_sale_id: string; }) => item.plant_workshop_ad_sale_id === this.adsId);
...
updateSelectOptions() {
  ...
  async saveUser(): Promise<void> {
    ...
    if (this.userForm.valid) {
      ...
      this.plantservice.updateWorkshopBusiness(formData).subscribe(
        async (response) => {
