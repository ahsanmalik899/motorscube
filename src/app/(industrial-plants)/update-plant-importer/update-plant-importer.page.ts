...
@Component({
  selector: 'app-update-plant-importer',
  templateUrl: './update-plant-importer.page.html',
  styleUrls: ['./update-plant-importer.page.scss'],
  standalone:false,
})
export class UpdatePlantImporterPage implements OnInit {
    ...
    fetchCarSale() {
      ...
      this.plantservice.getimporterData().subscribe({
        next: (data) => {
          ...
          this.filteredCarSaleData = this.carSaleData.filter((item: {plant_importer_ad_sale_id: string; }) => item.plant_importer_ad_sale_id === this.adsId);
...
updateSelectOptions() {
  ...
  submitFormData(formData: FormData, loading: any): void {
    this.plantservice.updateImporterBusiness(formData).subscribe(
      (response) => {
