...
@Component({
  selector: 'app-plant-exporter-single-view',
  templateUrl: './plant-exporter-single-view.page.html',
  styleUrls: ['./plant-exporter-single-view.page.scss'],
  standalone:false,
})
export class PlantExporterSingleViewPage implements OnInit {
  ...
  filterCarData() {
    ...
    this.filterData = this.exporterData.filter(car =>
      (exporterId.length === 0 || exporterId === car.plant_exporter_ad_sale_id) // Exact match check
    );
