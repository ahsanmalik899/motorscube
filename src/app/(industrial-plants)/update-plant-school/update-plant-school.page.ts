...
@Component({
  selector: 'app-update-plant-school',
  templateUrl: './update-plant-school.page.html',
  styleUrls: ['./update-plant-school.page.scss'],
  standalone:false,
})
export class UpdatePlantSchoolPage implements OnInit {
    ...
    fetchCarSale() {
      ...
      this.plantservice.getschoolData().subscribe({
        next: (data) => {
          ...
          this.filteredCarSaleData = this.carSaleData.filter((item: {plant_school_ad_sale_id: string; }) => item.plant_school_ad_sale_id === this.adsId);
          ...
          if (this.filteredCarSaleData.length > 0) {
            ...
            this.SchoolFeatured= carData.school_featured_type;
              this.fetchsubtype(carData.school_sector);
          ...
          this.userForm.patchValue({
            ...
            bendTime: carData.school_time_to || '',
             type: carData.school_sector || '',
            subtype: carData.school_subsector || '',
          });
...
updateSelectOptions() {
          ...
          switch (controlName) {
            ...
            case 'bendTime':
              ...
              case 'dealIn' :
                ...
                break;
                   case 'type':
                    invalidFields.push('School Sector');
                    break;
                case 'subtype':
            invalidFields.push('School Sub Sector');
            break;
            default:
              break;
          }
  ...
  submitFormData(loading: HTMLIonLoadingElement): void {
    ...
    this.plantservice.updateDrivingSchool(formData).subscribe(
      (response) => {
