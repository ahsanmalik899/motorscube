...
@Component({
  selector: 'app-post-plant-insurance',
  templateUrl: './post-plant-insurance.page.html',
  styleUrls: ['./post-plant-insurance.page.scss'],
  standalone:false,
})
export class PostPlantInsurancePage implements OnInit {
...
selectedFileArray: FileList | null = null;
  ...
  constructor(private formBuilder: FormBuilder,private plantservice:PlantsService, private userService: UserService, private alertController: AlertController, private router: ActivatedRoute,
...
async saveUser(): Promise<void> {
  ...
  if (this.userForm.valid) {
    ...
    this.plantservice.postInsuranceBusiness(formData).subscribe(
      (response) => {
