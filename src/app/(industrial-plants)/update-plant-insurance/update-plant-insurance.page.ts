...
@Component({
  selector: 'app-update-plant-insurance',
  templateUrl: './update-plant-insurance.page.html',
  styleUrls: ['./update-plant-insurance.page.scss'],
  standalone:false,
})
export class UpdatePlantInsurancePage implements OnInit {
    ...
    fetchCarSale() {
      ...
      this.plantservice.getinsuranceData().subscribe({
        next: (data) => {
          ...
          this.filteredCarSaleData = this.carSaleData.filter((item: {plant_insurance_ad_sale_id: string; }) => item.plant_insurance_ad_sale_id === this.adsId);
...
updateSelectOptions() {
 ...
 togglePassword(): void {
  ...
  async submitFormData(formData: FormData, loading: any): Promise<void> {
    try {
      const response = await this.plantservice.updateInsuranceBusiness(formData).toPromise();
