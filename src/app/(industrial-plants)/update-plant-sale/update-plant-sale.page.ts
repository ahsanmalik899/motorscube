...
@Component({
  selector: 'app-update-plant-sale',
  templateUrl: './update-plant-sale.page.html',
  styleUrls: ['./update-plant-sale.page.scss'],
  standalone: false,
})
export class UpdatePlantSalePage implements OnInit {
  options = {
    centeredSlides: true,
    slidesPerView: 3.5,
    spaceBetween: -30,
  ...
  constructor(
    ...
    private alercontroler: AlertController,
  ...
  fetchCarSale() {
    ...
    this.plantservice.getPlantSale().subscribe({
      next: (data) => {
        ...
        this.filteredCarSaleData = this.carSaleData.filter(
          (item: { plant_ad_sale_id: string }) =>
            item.plant_ad_sale_id === this.adsId
        );
        ...
        if (this.filteredCarSaleData.length > 0) {
          ...
          this.selectedTransmission =
            this.filteredCarSaleData[0].vehicle_power_transmission;
          this.selectedDrive = this.filteredCarSaleData[0].vehicle_transmission;
          this.carAdNormalFeature = this.filteredCarSaleData[0].ad_type;
          ...
          Object.keys(this.filteredCarSaleData[0]).forEach((key) => {
            if (
              key.startsWith('car_image') &&
              this.filteredCarSaleData[0][key] !== 'NULL'
            ) {
