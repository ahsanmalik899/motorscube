...
@Component({
  selector: 'app-update-plant-showroom',
  templateUrl: './update-plant-showroom.page.html',
  styleUrls: ['./update-plant-showroom.page.scss'],
  standalone:false,
})
export class UpdatePlantShowroomPage implements OnInit {
    ...
    fetchCarSale() {
      ...
      this.plantservice.getshowroomData().subscribe({
        next: (data) => {
          ...
          this.filteredCarSaleData = this.carSaleData.filter((item: {plant_showroom_ad_sale_id: string; }) => item.plant_showroom_ad_sale_id=== this.adsId);
...
updateSelectOptions() {
  ...
  async saveUser(): Promise<void> {
    ...
    if (this.userForm.valid) {
      ...
      this.plantservice.updateShowroomBusiness(formData).subscribe(
        async (response) => {
