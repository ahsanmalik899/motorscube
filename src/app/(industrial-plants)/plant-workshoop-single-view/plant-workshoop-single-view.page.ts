...
@Component({
  selector: 'app-plant-workshoop-single-view',
  templateUrl: './plant-workshoop-single-view.page.html',
  styleUrls: ['./plant-workshoop-single-view.page.scss'],
  standalone:false,
})
export class PlantWorkshoopSingleViewPage implements OnInit {
  ...
  constructor(public router: Router,private PlantsService:PlantsService, private popoverController: PopoverController, private userService: UserService,
  ...
  fetchworkshopData() {
    ...
    this.PlantsService.getworkshopData().subscribe({
      next: (data) => {
  ...
  filterCarData() {
    ...
    this.filterData = this.workshopData.filter(car => {
      ...
      return workshopId && workshopId === car.plant_workshop_ad_sale_id.toString();
    });
