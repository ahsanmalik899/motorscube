...
@Component({
  selector: 'app-machinery-you-posted',
  templateUrl: './machinery-you-posted.page.html',
  styleUrls: ['./machinery-you-posted.page.scss'],
  standalone:false,
})
export class MachineryYouPostedPage implements OnInit {
  postItem: any;
  closeDialog() {
  ...
  this.userID=localStorage.getItem('userId')?? '';
    ...
    checksalePaymentStatus(saleItemId: string,postType:string): boolean {
      ...
      for (let saleItem of this.sale) {
        ...
        if (saleItem.post_status === 'Pending' && saleItem.post_id.toString() === saleItemId && saleItem.post_type === postType) {
    ...
    checkhirePaymentStatus(saleItemId: string,postType:string): boolean {
      // Initialize the flag to false before checking
      let isButtonVisible = false;
    ...
    checksaleUpgradeStatus(saleItemId: string,postType:string): boolean {
      // Initialize the flag to false before checking
      let isUpgradeButtonVisible = true;
    
      // Loop through all items in the sale array
      for (let saleItem of this.sale) {
      if(saleItem.post_id.toString() === saleItemId&& saleItem.post_type === postType){      // Check if post_status is 'Pending' and post_id matches the saleItemId
    ...
    checkhireUpgradeStatus(saleItemId: string,postType:string): boolean {
      let isUpgradeButtonVisible = true;
    
      // Loop through all items in the sale array
      for (let saleItem of this.hire) {
      if(saleItem.post_id.toString() === saleItemId){      // Check if post_status is 'Pending' and post_id matches the saleItemId
        if (saleItem.post_status !== 'Pending' && saleItem.post_type===postType) {
          isUpgradeButtonVisible = true; // Set to true if the condition is met
          console.log('true1')
          break; // Stop checking once a matching item is found
  ...
  async buttonOneAction(id: string, saletype: string) {
    ...
    async editSale(id: any){
      this.router.navigate(['/update-machinery-sale'], {
        queryParams: {
          adsId: id,
        }
      });
    ...
    async editHire(id: any){
      this.router.navigate(['/update-machinery-hire'], {
        queryParams: {
          adsId: id,
        }
      });
