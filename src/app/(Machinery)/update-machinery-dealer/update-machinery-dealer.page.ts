...
@Component({
  selector: 'app-update-machinery-dealer',
  templateUrl: './update-machinery-dealer.page.html',
  styleUrls: ['./update-machinery-dealer.page.scss'],
  standalone:false,
})
export class UpdateMachineryDealerPage implements OnInit {
  DealIns!: string[];
  back() {
  history.back();
  }
   ...
   DealershipFeatured='';
      ...
      constructor(private machineryservice:MachineryService, private formBuilder: FormBuilder, private userService: UserService, private alertController: AlertController,
      ...
      fetchCarSale() {
        ...
        this.machineryservice.getdealershipData().subscribe({
          next: (data) => {
            ...
            this.filteredCarSaleData = this.carSaleData.filter((item: {machinery_dealership_ad_sale_id: string; }) => item.machinery_dealership_ad_sale_id === this.adsId);
  ...
  updateSelectOptions() {
    ...
    async saveUser(): Promise<void> {
      ...
      if (this.userForm.valid) {
        ...
        this.machineryservice.updateDealerBusiness(this.adsId, formData).subscribe(
          (response) => {
