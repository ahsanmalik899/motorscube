...
@Component({
  selector: 'app-update-machinery-insurance',
  templateUrl: './update-machinery-insurance.page.html',
  styleUrls: ['./update-machinery-insurance.page.scss'],
  standalone:false,
})
export class UpdateMachineryInsurancePage implements OnInit {
    ...
    fetchCarSale() {
      ...
      this.machineryservice.getinsuranceData().subscribe({
        next: (data) => {
          ...
          this.filteredCarSaleData = this.carSaleData.filter((item: {machinery_insurance_ad_sale_id: string; }) => item.machinery_insurance_ad_sale_id === this.adsId);
          ...
          this.userForm.patchValue({
            ...
            binsurance: carData.insurance_cover || '',
             dealIn: carData.insurance_deals_in || ''
          });
...
updateSelectOptions() {
 ...
 togglePassword(): void {
  ...
  async submitFormData(formData: FormData, loading: any): Promise<void> {
    try {
      const response = await this.machineryservice.updateInsuranceBusiness(formData).toPromise();
