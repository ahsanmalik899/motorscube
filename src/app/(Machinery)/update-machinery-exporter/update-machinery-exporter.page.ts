...
import { MachineryBusenessPage } from '../machinery-buseness/machinery-buseness.page';
...
@Component({
  selector: 'app-update-machinery-exporter',
  templateUrl: './update-machinery-exporter.page.html',
  styleUrls: ['./update-machinery-exporter.page.scss'],
  standalone:false,
})
export class UpdateMachineryExporterPage implements OnInit {
  DealIns!: string[];
    ...
    fetchCarSale() {
      ...
      this.machineryservice.getexporterData().subscribe({
        next: (data) => {
          ...
          this.filteredCarSaleData = this.carSaleData.filter((item: {machinery_exporter_ad_sale_id: string; }) => item.machinery_exporter_ad_sale_id === this.adsId);
...
updateSelectOptions() {
  ...
  async saveUser(): Promise<void> {
    ...
    if (this.userForm.valid) {
      ...
      this.machineryservice.updateExporterBusiness(formData).subscribe(
        (response) => {
