...
@Component({
  selector: 'app-update-machinery-leasing',
  templateUrl: './update-machinery-leasing.page.html',
  styleUrls: ['./update-machinery-leasing.page.scss'],
  standalone:false,
})
export class UpdateMachineryLeasingPage implements OnInit {
    ...
    this.machineryservice.getleasingData().subscribe({
      next: (data) => {
       ...
       // Store fetched data in carData property
        this.filteredCarSaleData = this.carSaleData.filter((item: {machinery_leasing_ad_sale_id: string; }) => item.machinery_leasing_ad_sale_id === this.adsId);
...
async saveUser(): Promise<void> {
  ...
  if (this.userForm.valid) {
    ...
    // Now you can use fileList for your API call
    this.machineryservice.updateLeasingBusiness(formData).subscribe(
      (response) => {
