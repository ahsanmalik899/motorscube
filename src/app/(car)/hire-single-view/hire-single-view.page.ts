...
@Component({
    selector: 'app-hire-single-view',
    templateUrl: './hire-single-view.page.html',
    styleUrls: ['./hire-single-view.page.scss'],
    standalone: false
})
export class HireSingleViewPage implements OnInit {
  isReadMore = true;
  imageUrls: string[] = [];
  selectedImage = '';
  slideOpts = {
...
isModalOpen = false;
  ...
  fetchCarSale() {
    this.userService.getSingleCarhire(this.saleID).subscribe({
      next: (data: any[]) => {
