...
@Component({
    selector: 'app-insurance-single-view',
    templateUrl: './Insurance-single-view.page.html',
    styleUrls: ['./Insurance-single-view.page.scss'],
    standalone: false
})
export class InsuranceSingleViewPage implements OnInit {
  isReadMore = true;
  ...
  filterCarData() {
    ...
    this.filterData = this.insuranceData.filter(car =>
      (insuranceId.length === 0 || insuranceId === car.car_insurance_ad_sale_id)
    );
