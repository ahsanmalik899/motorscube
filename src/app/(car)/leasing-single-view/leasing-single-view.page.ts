...
@Component({
    selector: 'app-leasing-single-view',
    templateUrl: './leasing-single-view.page.html',
    styleUrls: ['./leasing-single-view.page.scss'],
    standalone: false
})
export class LeasingSingleViewPage implements OnInit {
  isReadMore = true;
  ...
  filterCarData() {
    ...
    this.filterData = this.leasingData.filter(car =>
      (leasingId.length === 0 || leasingId === car.car_leasing_ad_sale_id)
    );
