...
@Component({
    ...
    templateUrl: './car-hire.page.html',
    styleUrls: ['./car-hire.page.scss'],
    standalone: false
})

export class CarHirePage implements OnInit {
...
selectedFileArray: FileList | null = null;
  ...
  userType='';
   ...
   constructor(private alertController: AlertController, public route: Router, private popoverController: PopoverController, private userService: UserService,  private loadingController: LoadingController,) {
...
async postAd() {
  ...
  if (this.selectedFiles && this.selectedFiles.length > 0) {
    ...
    try {
      ...
      const response = await this.userService.carHirePost(formData).toPromise();
      console.log('Data saved successfully:', response);
      this.presentSuccessAlert();
    } catch (error) {
      console.error('Error saving data:', error);
      this.presentErrorAlert('There was an issue posting your ad. Please try again.');
    } finally {
...
function getItems(ev: any, Event: { new(type: string, eventInitDict?: EventInit): Event; prototype: Event; readonly NONE: 0; readonly CAPTURING_PHASE: 1; readonly AT_TARGET: 2; readonly BUBBLING_PHASE: 3; }) {
