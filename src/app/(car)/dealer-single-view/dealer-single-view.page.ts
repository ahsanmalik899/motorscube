...
@Component({
    selector: 'app-dealer-single-view',
    templateUrl: './dealer-single-view.page.html',
    styleUrls: ['./dealer-single-view.page.scss'],
    standalone: false
})
export class DealerSingleViewPage implements OnInit {
  isReadMore = true;
  ...
  filterCarData() {
    ...
    this.filterData = this.dealershipData.filter(car => {
      ...
      return (dealerId.length === 0 || car.car_dealership_ad_sale_id === dealerId);
    });
