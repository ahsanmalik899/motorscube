...
@Component({
    selector: 'app-importer-single-view',
    templateUrl: './importer-single-view.page.html',
    styleUrls: ['./importer-single-view.page.scss'],
    standalone: false
})
export class ImporterSingleViewPage implements OnInit {
  isReadMore = true;
  ...
  filterCarData() {
    ...
    this.filterData = this.importerData.filter(car =>
      (importerId.length === 0 || importerId === car.car_importer_ad_sale_id) // Exact match check
    );
