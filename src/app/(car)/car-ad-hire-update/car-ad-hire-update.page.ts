...
@Component({
    selector: 'app-car-ad-hire-update',
    templateUrl: './car-ad-hire-update.page.html',
    styleUrls: ['./car-ad-hire-update.page.scss'],
    standalone: false
})
export class CarAdHireUpdatePage implements OnInit {
...
selectedFileArray: FileList | null = null;
 ...
 @ViewChild('selectedImage') selectedImage!: ElementRef<HTMLImageElement>;
   ...
   constructor(public route: Router, private popoverController: PopoverController, private userService: UserService, private alertcontroler:AlertController,
  ...
  fetchCarHire() {
    ...
    this.userService.getCarHire().subscribe({
      next: (data) => {
        ...
        this.filteredCarSaleData = this.carSaleData.filter((item: { car_ad_hire_id: string; }) => item.car_ad_hire_id === this.adsId);
        ...
        if (this.filteredCarSaleData.length > 0) {
          ...
          this.price=this.filteredCarSaleData[0].car_charges;
          this.selectedDuration = this.filteredCarSaleData[0].car_charges_type;
          this.selectdriver = this.filteredCarSaleData[0].car_driver;
...
async updateAd() {
  ...
  try {
    const response = await this.userService.carHireUpdate(formData).toPromise();
