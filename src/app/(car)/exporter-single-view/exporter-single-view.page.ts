...
@Component({
    selector: 'app-exporter-single-view',
    templateUrl: './exporter-single-view.page.html',
    styleUrls: ['./exporter-single-view.page.scss'],
    standalone: false
})
export class ExporterSingleViewPage implements OnInit {
  isReadMore = true;
  saleUser='';
  exporterData: any[] = [];
  filterUserData: any[] = [];
  ...
  filterCarData() {
    ...
    this.filterData = this.exporterData.filter(car =>
      (exporterId.length === 0 || exporterId === car.car_exporter_ad_sale_id) // Exact match check
    );
