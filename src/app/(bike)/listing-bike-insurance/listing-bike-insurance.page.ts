...
@Component({
  selector: 'app-listing-bike-insurance',
  templateUrl: './listing-bike-insurance.page.html',
  styleUrls: ['./listing-bike-insurance.page.scss'],
  standalone:false,
})
export class ListingBikeInsurancePage implements OnInit {
insuranceData: any[] = [];
  selectedcon: string[] = [];
  selectedcity: string[] = [];
...
fetchinsuranceData() {
  ...
  this.bikeservice.getinsuranceData().subscribe({
    next: (data: any[]) => {
      ...
      localStorage.setItem('bikeinsuranceData', JSON.stringify(data));
