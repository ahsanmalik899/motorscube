...
@Component({
  selector: 'app-update-bike-importer-page',
  templateUrl: './update-bike-importer-page.page.html',
  styleUrls: ['./update-bike-importer-page.page.scss'],
  standalone:false,
})
export class UpdateBikeImporterPagePage implements OnInit {
      ...
      fetchCarSale() {
        ...
        this.bikeservice.getimporterData().subscribe({
          next: (data) => {
            ...
            this.filteredCarSaleData = this.carSaleData.filter((item: { bike_importer_ad_sale_id: string; }) => item.bike_importer_ad_sale_id === this.adsId);
  ...
  updateSelectOptions() {
    ...
    submitFormData(formData: FormData, loading: any): void {
      this.bikeservice.updateImporterBusiness(formData).subscribe(
        (response) => {
