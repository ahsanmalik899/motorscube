...
@Component({
  selector: 'app-update-vehicle-dealer',
  templateUrl: './update-vehicle-dealer.page.html',
  styleUrls: ['./update-vehicle-dealer.page.scss'],
  standalone:false,
})
export class UpdateVehicleDealerPage implements OnInit {
  ...
  history.back();
   ...
   DealershipFeatured='';
      ...
      constructor(private commercialservice:CommercialService, private formBuilder: FormBuilder, private userService: UserService, private alertController: AlertController,
      ...
      fetchCarSale() {
        ...
        this.commercialservice.getdealershipData().subscribe({
          next: (data) => {
            ...
            this.filteredCarSaleData = this.carSaleData.filter((item: {commercial_vehicle_dealership_ad_sale_id: string; }) => item.commercial_vehicle_dealership_ad_sale_id === this.adsId);
  ...
  updateSelectOptions() {
    ...
    async saveUser(): Promise<void> {
      ...
      if (this.userForm.valid) {
        ...
        this.commercialservice.updateDealerBusiness(this.adsId, formData).subscribe(
          (response) => {
