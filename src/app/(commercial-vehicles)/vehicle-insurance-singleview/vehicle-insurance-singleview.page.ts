...
@Component({
  selector: 'app-vehicle-insurance-singleview',
  templateUrl: './vehicle-insurance-singleview.page.html',
  styleUrls: ['./vehicle-insurance-singleview.page.scss'],
  standalone:false,
})
export class VehicleInsuranceSingleviewPage implements OnInit {
  ...
  filterCarData() {
    ...
    this.filterData = this.insuranceData.filter(car =>
      (insuranceId.length === 0 || insuranceId === car.commercial_vehicle_insurance_ad_sale_id)
    );
