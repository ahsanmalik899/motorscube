...
@Component({
  selector: 'app-post-vehicle-insurance',
  templateUrl: './post-vehicle-insurance.page.html',
  styleUrls: ['./post-vehicle-insurance.page.scss'],
  standalone:false,
})
export class PostVehicleInsurancePage implements OnInit {
  DealIns!: string[];
back() {
history.back();
}
...
async saveUser(): Promise<void> {
  ...
  if (this.userForm.valid) {
    ...
    this.commercialservice.postInsuranceBusiness(formData).subscribe(
      (response) => {
