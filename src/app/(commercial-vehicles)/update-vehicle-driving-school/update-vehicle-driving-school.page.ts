    ...
    fetchCarSale() {
      ...
      this.userService.getschoolData().subscribe({
        next: (data) => {
          ...
          this.filteredCarSaleData = this.carSaleData.filter((item: { car_school_ad_sale_id: string; }) => item.car_school_ad_sale_id === this.adsId);
...
updateSelectOptions() {
  ...
  submitFormData(loading: HTMLIonLoadingElement): void {
    ...
    this.userService.updateDrivingSchool(formData).subscribe(
      (response) => {
