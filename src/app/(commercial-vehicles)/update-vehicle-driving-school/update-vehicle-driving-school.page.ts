...
@Component({
  selector: 'app-update-vehicle-driving-school',
  templateUrl: './update-vehicle-driving-school.page.html',
  styleUrls: ['./update-vehicle-driving-school.page.scss'],
  standalone:false,
})
export class UpdateVehicleDrivingSchoolPage implements OnInit {
    ...
    fetchCarSale() {
      ...
      this.commercialservice.getschoolData().subscribe({
        next: (data) => {
          ...
          this.filteredCarSaleData = this.carSaleData.filter((item: {commercial_school_ad_sale_id: string; }) => item.commercial_school_ad_sale_id === this.adsId);
...
updateSelectOptions() {
  ...
  submitFormData(loading: HTMLIonLoadingElement): void {
    ...
    this.commercialservice.updateDrivingSchool(formData).subscribe(
      (response) => {
